...
class CourtRegisterUpdateService(
  ...
  fun updateCourtDetails(court: CourtUpdate): SyncStatistics {
    ...
    courtRegisterService.getCourtInfoFromRegister(court.courtId)?.run {
      ...
      // The probationAreaCode should be calculated from the Address or through an API before the sync process
      // Until this is determined, copy over the ProbationAreaCode from the corresponding Probation Court Data (if it exists)
      val registerPayload = if (courtFromProbation != null) courtWithProbationArea(CourtDataToSync(this), courtFromProbation) else CourtDataToSync(this)
      val probationPayload = if (courtFromProbation != null) CourtDataToSync(courtFromProbation) else null
  ...
  fun courtWithProbationArea(courtDataToSync: CourtDataToSync, courtFromProbation: ProbationService.CourtFromProbationSystem): CourtDataToSync =
    courtFromProbation?.let { courtDataToSync.copy(probationAreaCode = courtFromProbation.probationArea.code) }
